...
// Prefer tfjs-node if available (faster native bindings). Fall back to pure JS @tensorflow/tfjs for environments
// where native builds are not available (e.g., Windows without build tools or unsupported Node versions).
let tf;
try {
  tf = require('@tensorflow/tfjs-node');
  logger.info('Using @tensorflow/tfjs-node native bindings');
} catch (err) {
  tf = require('@tensorflow/tfjs');
  logger.info('Falling back to @tensorflow/tfjs (pure JS). Tensor operations may be slower.');
}
